...
class ApiTargetCategoriesTest extends \Parents\Tests\PhpUnit\ApiTestCase
{
    ...
    {
        $this->auth();
        $response = $this->json('GET', route('api.target-categories.index'));
    ...
    {
        $this->auth();
        $response = $this->json('GET', route('api.target-categories.show', ['target_category' => 1]));
        $response->assertStatus(200)->assertJson(['data' => [
            "id" => 1
        ]]);
    }
    ...
    {
        $this->auth();
        $response = $this->postJson( route('api.target-categories.store'), ['target_category_name' => 'Test Category', 'target_category_type' => 1]);
    ...
    {
        $this->auth();
        $response = $this->putJson(route('api.target-categories.update', ['target_category' => 1]), ['target_category_name' => 'Changed name', 'target_category_type' => 1]);
