...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.target-categories.store") }}" enctype="multipart/form-data">
            @csrf
            <div class="form-group">
                <label class="required" for="target_category_name">{{ trans('cruds.targetCategory.fields.target_category_name') }}</label>
                <input class="form-control {{ $errors->has('target_category_name') ? 'is-invalid' : '' }}" type="text" name="target_category_name" id="target_category_name" value="{{ old('target_category_name', '') }}" required>
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('target_category_type') ? 'is-invalid' : '' }}" name="target_category_type" id="target_category_type" required>
                    ...
                    @foreach(App\Models\TargetCategory::TARGET_CATEGORY_TYPE_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('target_category_type', '2') === (string) $key ? 'selected' : '' }}>{{ trans('global.' . $label) }}</option>
                    @endforeach
                </select>
