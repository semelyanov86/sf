            ...
            <div class="form-group">
                ...
                @foreach(\Domains\Targets\Enums\TargetStatusEnum::getInstances() as $key => $label)
                    <div class="form-check {{ $errors->has('target_status') ? 'is-invalid' : '' }}">
                        <input class="form-check-input" type="radio" id="target_status_{{ $label->value }}" name="target_status" value="{{ $label->value }}" {{ old('target_status', '0') === (string) $label->value ? 'checked' : '' }} required>
