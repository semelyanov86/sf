        ...
        <form method="POST" action="{{ route("admin.categories.store") }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('type') ? 'is-invalid' : '' }}" name="type" id="type">
                    ...
                    @foreach(\Domains\Categories\Enums\CategoryTypeEnum::getInstances() as $key => $label)
                        <option value="{{ $label->value }}" {{ old('type', '1') === (string) $label->value ? 'selected' : '' }}>{{ $label->description }}</option>
                    @endforeach
                </select>
                @if($errors->has('type'))
                    <div class="invalid-feedback">
            ...
            <div class="form-group">
                <div class="form-check {{ $errors->has('is_hidden') ? 'is-invalid' : '' }}">
                    ...
                    <input class="form-check-input" type="checkbox" name="is_hidden" id="is_hidden" value="1" {{ old('is_hidden', 0) == 1 ? 'checked' : '' }}>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('sys_category') ? 'is-invalid' : '' }}" type="number" name="sys_category" id="sys_category" value="{{ old('sys_category', '') }}" step="1">
            ...
            <div class="form-group">
                ...
                <input class="form-control datetime {{ $errors->has('last_used_at') ? 'is-invalid' : '' }}" type="text" name="last_used_at" id="last_used_at" value="{{ old('last_used_at') }}">
