        ...
        <div class="col-md-12">
            @can('auto_brand_create')
                <div style="margin-bottom: 10px;" class="row">
                    <div class="col-lg-12">
                        <a class="btn btn-success" href="{{ route('frontend.auto-brands.create') }}">
                            ...
                            <tbody>
                                @foreach($autoBrands as $key => $autoBrand)
                                    <tr data-entry-id="{{ $autoBrand->id }}">
                                        ...
                                        <td>
                                            @can('auto_brand_show')
                                                <a class="btn btn-xs btn-primary" href="{{ route('frontend.auto-brands.show', $autoBrand->id) }}">
                                                    {{ trans('global.view') }}
                                                </a>
                                            @endcan

                                            @can('auto_brand_edit')
                                                <a class="btn btn-xs btn-info" href="{{ route('frontend.auto-brands.edit', $autoBrand->id) }}">
                                                    {{ trans('global.edit') }}
                                                </a>
                                            @endcan

                                            @can('auto_brand_delete')
                                                <form action="{{ route('frontend.auto-brands.destroy', $autoBrand->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('auto_brand_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('frontend.auto-brands.massDestroy') }}",
