        ...
        <div class="col-md-12">
            @can('operation_create')
                <div style="margin-bottom: 10px;" class="row">
                    <div class="col-lg-12">
                        <a class="btn btn-success" href="{{ route('frontend.operations.create') }}">
                            {{ trans('global.add') }} {{ trans('cruds.operation.title_singular') }}
                        </a>
                        <button class="btn btn-warning" data-toggle="modal" data-target="#csvImportModal">
                            {{ trans('global.app_csvImport') }}
                        </button>
                        @include('csvImport.modal', ['model' => 'Operation', 'route' => 'admin.operations.parseCsvImport'])
                    </div>
                </div>
            @endcan
            <div class="card">
                            ...
                            <tbody>
                                @foreach($operations as $key => $operation)
                                    <tr data-entry-id="{{ $operation->id }}">
                                        ...
                                        <td>
                                            @can('operation_show')
                                                <a class="btn btn-xs btn-primary" href="{{ route('frontend.operations.show', $operation->id) }}">
                                                    {{ trans('global.view') }}
                                                </a>
                                            @endcan

                                            @can('operation_edit')
                                                <a class="btn btn-xs btn-info" href="{{ route('frontend.operations.edit', $operation->id) }}">
                                                    {{ trans('global.edit') }}
                                                </a>
                                            @endcan

                                            @can('operation_delete')
                                                <form action="{{ route('frontend.operations.destroy', $operation->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('operation_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('frontend.operations.massDestroy') }}",
