        ...
        <div class="col-md-12">
            @can('account_create')
                <div style="margin-bottom: 10px;" class="row">
                    <div class="col-lg-12">
                        <a class="btn btn-success" href="{{ route('frontend.accounts.create') }}">
                            ...
                            <tbody>
                                @foreach($accounts as $key => $account)
                                    <tr data-entry-id="{{ $account->id }}">
                                        ...
                                        <td>
                                            @can('account_show')
                                                <a class="btn btn-xs btn-primary" href="{{ route('frontend.accounts.show', $account->id) }}">
                                                    {{ trans('global.view') }}
                                                </a>
                                            @endcan

                                            @can('account_edit')
                                                <a class="btn btn-xs btn-info" href="{{ route('frontend.accounts.edit', $account->id) }}">
                                                    {{ trans('global.edit') }}
                                                </a>
                                            @endcan

                                            @can('account_delete')
                                                <form action="{{ route('frontend.accounts.destroy', $account->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('account_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('frontend.accounts.massDestroy') }}",
